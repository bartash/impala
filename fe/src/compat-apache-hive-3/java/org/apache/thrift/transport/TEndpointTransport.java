...
public abstract class TEndpointTransport extends TTransport {
  ...
  /**
   * Updates RemainingMessageSize to reflect then known real message size (e.g. framed
   * transport). Will throw if we already consumed too many bytes or if the new size is
   * larger than allowed.
   * @param size
   */
  ...
  /**
   * Throws if there are not enough bytes in the input stream to satisfy a read of
